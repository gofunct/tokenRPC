...
type Command struct {
	...
	// ShouldInsideApp will disable the command when a current working directory is not inside of a hack project.
	...
	cc := &cobra.Command{
		...
		PreRunE: func(_ *cobra.Command, args []string) error {
			if c.ShouldInsideApp && !c.Ctx().IsInsideApp() {
				return errors.New("should execute inside hack project")
			}
			if c.PreRun != nil {
				err := c.PreRun(c, args)
				if err != nil {
