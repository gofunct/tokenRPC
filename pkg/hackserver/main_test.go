...
import (
	...
	"github.com/gofunct/hack/pkg/hackserver/testing/api"
	"github.com/gofunct/hack/pkg/hackserver/testing/app/server"
)
...
func startServer(t *testing.T, s *hackserver.Engine) func() {
...
func Test_server_onlyGateway(t *testing.T) {
	...
	s := hackserver.New(
		hackserver.WithGatewayAddr("tcp", ":"+strconv.FormatInt(port, 10)),
...
func Test_server_differentPort(t *testing.T) {
	...
	s := hackserver.New(
		hackserver.WithGrpcAddr("tcp", grpcAddr),
		hackserver.WithGatewayAddr("tcp", httpAddr),
		hackserver.WithServers(
			server.NewLibraryServiceServer(),
		),
	)
