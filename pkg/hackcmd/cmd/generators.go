...
func newGenerateCommands(ctx *hackcmd.Ctx) (cmds []*cobra.Command) {
	gCmd := &cobra.Command{
	...
	go func() {
		defer wg.Done()
		execs = fs.ListExecutableWithPrefix(ctx.FS, "hack-gen-")
	}()
	...
	go func() {
		...
		for _, t := range tools {
			if !strings.HasPrefix(t.Name(), "hack-gen-") {
				continue
			}
...
func newGenerateCommandByTool(repo tool.Repository, t tool.Tool, subCmd string) *cobra.Command {
	cmd := &cobra.Command{
		Use:  strings.TrimPrefix(t.Name(), "hack-gen-"),
...
func newGenerateCommandByExec(execer excmd.Executor, exec, subCmd string) *cobra.Command {
	cmd := &cobra.Command{
		Use:  strings.TrimPrefix(exec, "hack-gen-"),
		Args: cobra.ArbitraryArgs,
		RunE: func(_ *cobra.Command, args []string) error {
