...
func Test_Integration(t *testing.T) {
	...
	run(t, srcDir, exec.Command("hack", args...))
	...
	run(t, rootPath, exec.Command("hack", "--debug", "g", "service", "book", "list"))
	...
	cmd := exec.CommandContext(svrCtx, "hack", "--debug", "server")
	cmd.Dir = rootPath
				...
				if n.Tok == token.IMPORT {
					n.Specs = []ast.Spec{
						...
						&ast.ImportSpec{
							Path: &ast.BasicLit{
								Kind:  token.STRING,
								Value: strconv.Quote("github.com/gofunct/hack/pkg/hackserver"),
							},
						},
						&ast.ImportSpec{
