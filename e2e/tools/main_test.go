...
func TestGex_Add(t *testing.T) {
	t.Run("add first tool", func(t *testing.T) {
		checkCmd(t, exec.Command("tools", "--add", "github.com/grpc-ecosystem/grpc-gateway/protoc-gen-grpc-gateway"))
		snapshotManifest(t)
	})

	t.Run("add 2 tools", func(t *testing.T) {
		checkCmd(t, exec.Command("tools", "--add", "github.com/haya14busa/reviewdog/cmd/reviewdog", "--add", "golang.org/x/lint/golint"))
		snapshotManifest(t)
	})
	...
	t.Run("add tools that the tool has the same package has already been added", func(t *testing.T) {
		checkCmd(t, exec.Command("tools", "--add", "github.com/grpc-ecosystem/grpc-gateway/protoc-gen-swagger"))
		snapshotManifest(t)
	})

	t.Run("add tools included in the same package", func(t *testing.T) {
		checkCmd(t, exec.Command("tools", "--add", "github.com/gogo/protobuf/protoc-gen-gogo@v1.1.1", "--add", "github.com/gogo/protobuf/protoc-gen-gogofast"))
		snapshotManifest(t)
	})
...
`), 0755))
		checkCmd(t, exec.Command("tools", "--add", "github.com/golang/mock/mockgen"))
		snapshotManifest(t)
